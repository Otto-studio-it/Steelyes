  ...
  return (
    <MarketingShell pathname="/case-study">
      <section className="mx-auto max-w-3xl px-4 py-16 md:px-8 md:py-24">
        <p className="font-mono text-xs uppercase tracking-widest text-[#9E000C]">Case studies</p>
        <h1 className="mt-3 font-heading text-4xl font-black uppercase leading-[0.95] sm:text-5xl">
          Coming soon
        </h1>
        <p className="mt-5 text-base font-light leading-relaxed text-[#5C403D]">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <Link
            href="/contact"
            className="inline-flex min-h-[48px] items-center justify-center bg-[#9E000C] px-6 font-heading text-sm font-bold uppercase tracking-tight text-white"
          >
            Request a quote
          </Link>
          <a
            href={`mailto:${BUSINESS.email}?subject=${encodeURIComponent('Case study / project references')}`}
            className="inline-flex min-h-[48px] items-center justify-center border border-zinc-300 px-6 font-heading text-sm font-bold uppercase tracking-tight text-[#1B1C1A]"
          >

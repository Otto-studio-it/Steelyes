...
export function GateTypeExplorationDialog({
  ...
  const isEnquire = availability === 'enquire'
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <Dialog.Title className="font-heading text-lg font-black uppercase tracking-tight text-steel">
                {isEnquire ? 'Enquire to specify' : 'Exploration mode'}
              </Dialog.Title>
              <Dialog.Description className="mt-2 text-sm leading-6 text-muted-deep">
                {isEnquire ? (
                  <>
                    <strong className="font-semibold text-steel">{gateTypeLabel(gateType)}</strong> is not
                    available in the online configurator yet. Send an enquiry and we will specify it with you
                    after survey — we will not invent dimensions or prices here.
                  </>
                ) : (
                  <>
                    <strong className="font-semibold text-steel">{gateTypeLabel(gateType)}</strong> is outside
                    the primary Double Swing path. You can explore it with a schematic preview — visual fidelity
                    and pricing may be incomplete until survey.
                  </>
                )}
              </Dialog.Description>
            </div>

...
- Verified locally: typecheck, lint, engine 207 tests, web 56 tests, production build, Playwright `configurator` + `header-configurator` specs 34/34 (against the dev server + an in-memory REST stand-in); end-to-end AR flow against an in-memory REST stand-in (desktop, spoofed iPhone UA, share page). **Not verified:** real Quick Look / Scene Viewer on devices — needs a public HTTPS deploy.

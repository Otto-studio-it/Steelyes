...
- **Phase 3 (mobile selectors):** sticky Quick Path preview is a full-width live drawing on step 1 (compact chip on steps 2–3 / landscape) and replaces the duplicate hero; gate shape = large snap cards on phones; railheads keep their order (selected card scrolls into view), bigger cards, "See all N models" sheet; finish swatches 56px in 3 columns; selected cards get a tick + 2px border; gate type sheet is a 2-column grid with only exception badges; dimensions button moved into the preview header (it covered the caption); cookie banner sits above the action bar; customer-facing copy no longer leaks internal terms (CA-xx, "2D master", "Design CAD"); no text under 12px in the quick path. E2E fixture used the removed finish code `matte_black` (share-route tests could not pass) — fixed.
- **Phase 4 (engine correctness):** cut list — stiles cut to gate height (were leaf width), rails cut to one leaf between stiles (were ~2× the steel), top member added, upper pickets = per-leaf count × leaves (30 not 16 on a 3 m double), picket tube labelled 20 mm (was the 40 mm frame tube), bifold panels doubled, cantilever gets a counterbalance tail and no ground track (CA-05); railheads priced from the current width per CA-14 (stored quantity went stale on resize; bay cap raised 40 → 100 for 10 m tracked gates); installation-view arch is symmetric; non-integer dimensions are rounded instead of replaced by the preset; `validateGateConfig` checks posts / motorised and returns issues instead of throwing on malformed input; estimate states when fence panels or a custom RAL are not included; plan-view cantilever tail uses the 1/3 rule at every width. **Left for the client:** width datum (clear opening vs leaf) across technical drawing / mesh, circles count rule (£2.50 each is still not applied — `open.circles_count_rule`), whether the middle bar is standard or a £275 option, pricing of fence panels / posts / custom RAL.
- **Phase 5 (operations):** `Content-Security-Policy-Report-Only` + `/api/csp-report` log sink (promote to enforcing once quiet); `/embed/*` drops `X-Frame-Options` for an enforced `frame-ancestors` allowlist (`EMBED_ALLOWED_ORIGINS`, build-time; unset = not embeddable) and the embed snippet checks `event.origin` / `event.source`; quote PDF cached 5 min and no longer 500s raw on bad rows; Resend webhook ignores out-of-order events and no longer 500s without an API key; abandoned-design reminders: failed sends are counted and deprioritised (they used to block the queue for 14 days), one-click signed unsubscribe (`/api/unsubscribe`, `List-Unsubscribe` headers), migration `20260921130000_…`; designs saved with pre-July finish codes (`matte_black`, `zinc_grey`, `bronze`, `pearl_white`) open again; `purge_expired_personal_data()` defined but **not scheduled** (migration `20260921140000_…`). Coolify must schedule the reminder job — the repo only had a Vercel cron entry.
- Verified locally: typecheck, lint, engine 218 tests, web 60 tests, production build, Playwright `configurator` + `header-configurator` specs 34/34 (against the dev server + an in-memory REST stand-in); end-to-end AR flow against an in-memory REST stand-in (desktop, spoofed iPhone UA, share page). **Not verified:** real Quick Look / Scene Viewer on devices — needs a public HTTPS deploy.

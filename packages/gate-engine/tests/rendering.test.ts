...
describe('gate-engine rendering', () => {
  ...
  it('builds a sliding preview plan with a track and note', () => {
    ...
    expect(plan.primitives.some((primitive) => primitive.kind === 'line' && primitive.id === 'tracked-ground-rail')).toBe(true)
  ...
  it('draws tracked runback, guide, and bottom box details', () => {
    ...
    expect(plan.primitives.some((p) => p.id === 'tracked-guide-rollers')).toBe(true)
  ...
  it('draws cantilever carriage and foundation cues', () => {
    ...
    expect(plan.primitives.some((p) => p.id === 'cantilever-leaf-bottom-box')).toBe(true)
  ...
  it('draws radius leaf splits for wide openings', () => {
    ...
    expect(plan.primitives.some((p) => p.id === 'radius-hinge-1')).toBe(true)
    expect(plan.primitives.some((p) => p.id === 'radius-hinge-2')).toBe(true)
  })
})

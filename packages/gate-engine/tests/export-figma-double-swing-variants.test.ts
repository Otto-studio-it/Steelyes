...
const VARIANTS = [
  ...
  },
  {
    slug: 'dog_bars',
    title: 'Dog bars',
    apply: (config: GateConfig) => enable(config, 'dog_bars', 12),
  },
  {
    slug: 'arched_dog_bars',
    title: 'Arched + dog bars',
    apply: (config: GateConfig) => enable(enable(config, 'arched_top'), 'dog_bars', 12),
  },
  {
    slug: 'composite',
...
describe('export Figma double_swing variant SVGs', () => {
  it('writes the 5 silhouette masters into docs/frontend/2d-masters/double_swing/silhouettes', () => {
    ...
    for (const variant of VARIANTS) {
      const config = variant.apply(createGateConfig(createGatePreset('double_swing')))
